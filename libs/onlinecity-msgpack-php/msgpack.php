...
function msgpack_pack_o($input)
{
	...
	if (is_array($input)) {
		...
		foreach ($input as $key => $elm) {
			if ($isMap) $buf .= msgpack_pack_o($key);
			$buf .= msgpack_pack_o($elm);
		}
		return $buf;
 ...
 * @return mixed
 */
function msgpack_unpack_o($input)
{
	static $bigendian;
	if (!isset($bigendian)) $bigendian = (pack('S',1)==pack('n',1));
	...
	if ((($byte ^ "\x80") & "\xF0") == "\x00") {
		...
		$data = array();
		for($i=0;$i<$len;$i++) {
			$key = msgpack_unpack_o($input);
			$value = msgpack_unpack_o($input);
			$data[$key] = $value;
		}
		return $data;
